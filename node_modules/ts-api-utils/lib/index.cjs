...
function isNamedDeclarationWithName(node) {
  return (
    'name' in node && node.name !== undefined && node.name !== null && isDeclarationName(node.name)
  );
}
...
function isSuperPropertyAccessExpression(node) {
  return (
    ts9__default.default.isPropertyAccessExpression(node) && isSuperExpression(node.expression)
  );
}
...
function isEvolvingArrayType(type) {
  return (
    isObjectType(type) && isObjectFlagSet(type, ts9__default.default.ObjectFlags.EvolvingArray)
  );
}
  ...
  createOrReuseNamespaceScope(name, exported, ambient, hasExportStatement) {
    ...
    this.#innerScope.end((variable, key, scope) => {
      if (
        ...
        (!variable.exported &&
          (!this.#ambient || (this.#exports !== undefined && !this.#exports.has(key.text))))
      ) {
...
var RootScope = class extends AbstractScope {
  ...
  addVariable(identifier, name, selector, exported, domain) {
    ...
    this.#innerScope.end((value, key) => {
      value.exported ||=
        this.#exportAll || (this.#exports !== undefined && this.#exports.includes(key.text));
      value.inGlobalScope = this.global;
      return cb(value, key, this);
    });
    return super.end((value, key, scope) => {
      value.exported ||=
        scope === this && this.#exports !== undefined && this.#exports.includes(key.text);
...
var UsageWalker = class {
  ...
  #handleFunctionLikeDeclaration(node, cb, varCb) {
    ...
    const scope = (this.#scope =
      node.kind === ts9__default.default.SyntaxKind.FunctionExpression && node.name !== undefined
